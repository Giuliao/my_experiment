...
class SDNE(basic_model.Basic):
    def __init__(self, config):
        ...
        # define variables
        self.layers = len(config.structure)
        ...
        for i in range(self.layers - 1):
            name = "encoder" + str(i)
            self.W[name] = tf.Variable(tf.random_normal([structure[i], structure[i + 1]]), name=name)
            self.b[name] = tf.Variable(tf.zeros([structure[i + 1]]), name=name)
        structure.reverse()
        for i in range(self.layers - 1):
            name = "decoder" + str(i)
            self.W[name] = tf.Variable(tf.random_normal([structure[i], structure[i + 1]]), name=name)
        ...
        self.X = tf.placeholder("float", [None, config.structure[0]])
    ...
    def __make_loss(self, config):
        def get_1st_loss_link_sample(Y1, Y2):
